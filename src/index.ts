...
function createImage(t: number): Image {
  const grid = distanceFields.toShape(
    transforms.scale(0.1, 0.1)(distanceFields.gridLines),
    0.01
  );

  const pattern = transforms.scale(0.025, 0.025)(
    (x, y) => (Math.floor(x) % 2 == 0) == (Math.floor(y) % 2 == 0)
  )

  const shape1 = combiners.intesect(
    transforms.rotate(0.25 * Math.PI)(pattern),
    combiners.union(
      shapes.circle(0.25), shapes.circle(0.125,0, -0.2)
    )
  );
  const shape2 = distanceFields.toShape(distanceFields.rectFilled(-0.5, -0.5, 1, 1), 0.25);
  const shape3 = distanceFields.toShape(
    distanceFields.combine(
          distanceFields.point(-0.33 * Math.sin(0.001 * t), -0.33),
          distanceFields.circle(0.5)
    ), 
    0.05
  )
  ...
  const sine = distanceFields.toShape(
    distanceFields.fromFunctionNaiveApprox((x) => 0.5 * Math.sin(50 * x)),
    0.05
  );
  ...
  const image = combiners.layer(
    shapes.fill(sine, () => colors.black),
    move(
      shrink(
        shapes.fill(shape3, (x) => ([x * 127 + 127, 0, 0] as Color))
      )
    ),
    transforms.translate(0.5, 0)(
      shapes.fill(shape1, () => colors.blue)
    ),
    shapes.fill(shape2, colorWaves),
    shapes.fill(grid, () => [0x99, 0x99, 0x99]),
    shapes.fill(pattern, () => [0xdd, 0xdd, 0xdd]),
  )
...
function* sampleGenerator(x: number, y: number): Generator<[number, number], never> {
  yield [x + 0.5, y + 0.5];
  while (true) {
...
function render(canvas: HTMLCanvasElement, image: Image, samplesPerPixel: number = 1) {
